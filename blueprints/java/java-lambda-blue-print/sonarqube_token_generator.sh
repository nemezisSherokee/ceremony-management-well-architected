...
do
	...
	Result=$(curl -u $LOGIN_NAME:$LOGIN_PASSWD  -d "name=$TOKEN_NAME&login=$LOGIN_NAME" -X POST "http://localhost:9005/api/user_tokens/generate")
	...
	then
		  ...
		  #1- revoke the token
		  curl -u $LOGIN_NAME:$LOGIN_PASSWD  -d "name=$TOKEN_NAME&login=$LOGIN_NAME" -X POST "http://localhost:9005/api/user_tokens/revoke"
		  #2- create a new one
